...
class MainActivityV11 : Activity() {
    companion object {
    ...
    @Volatile private var preloadedSongIndex = -1

    private var preloadedStems: List<LoadedStem>? = null
    ...
    private fun buildUi(): ScrollView {
        ...
        content.addView(TextView(this).apply {
            text = "SEQUENCE PLAYER · 0.11"
            setTextColor(Color.rgb(145, 160, 178)); textSize = 12f
        })
        content.addView(TextView(this).apply {
            text = "Setlist + precarga rápida"
    ...
    private fun renderStemRows() {
        ...
        currentStems.forEach { stem ->
            ...
            val mute = Button(this).apply { text = if (stem.muted) "M ✓" else "M"; setOnClickListener { stem.muted = !stem.muted; text = if (stem.muted) "M ✓" else "M" } }
            val solo = Button(this).apply { text = if (stem.solo) "S ✓" else "S"; setOnClickListener { stem.solo = !stem.solo; text = if (stem.solo) "S ✓" else "S" } }
    ...
    private fun saveCurrentSong() {
        ...
        preloadNextSong(selectedSongIndex)
    ...
    private fun loadSong(index: Int) {
        ...
        currentStems.clear(); currentFrame = 0; totalFrames = 0
        ...
        val cached = synchronized(preloadLock) {
            if (preloadedSongIndex == index) preloadedStems else null
        }

        if (cached != null && cached.size == currentStems.size) {
            loadingSong = false
            currentStems.forEachIndexed { i, stem -> stem.loaded = cached[i] }
            synchronized(preloadLock) { preloadedSongIndex = -1; preloadedStems = null }
            renderStemRows(); validateLoadedStems()
            statusView.text = "${song.title} lista al instante."
        ...
        }.apply { name = "SequenceSongLoad11"; start() }
    }

    private fun loadSongStems(song: SongPreset): List<LoadedStem> {
        return song.stems.map { preset ->
            val uri = Uri.parse(preset.uri)
            loadPcm16Wav(uri, getDisplayName(uri) ?: preset.name)
        }
    }

    private fun preloadNextSong(currentIndex: Int) {
        if (songs.size < 2 || currentIndex !in songs.indices) return
        val nextIndex = if (currentIndex >= songs.lastIndex) 0 else currentIndex + 1
        val nextSong = songs[nextIndex]
        val localPreloadGeneration = ++preloadGeneration
        synchronized(preloadLock) { preloadedSongIndex = -1; preloadedStems = null }

        Thread {
            try {
                val loaded = loadSongStems(nextSong)
                if (localPreloadGeneration != preloadGeneration || selectedSongIndex != currentIndex) return@Thread
                synchronized(preloadLock) {
                    if (localPreloadGeneration == preloadGeneration && selectedSongIndex == currentIndex) {
                        preloadedSongIndex = nextIndex
                        preloadedStems = loaded
                    }
                }
                runOnUiThread {
                    if (localPreloadGeneration == preloadGeneration && selectedSongIndex == currentIndex && !playing) {
                        statusView.text = "${songs[currentIndex].title} lista · siguiente preparada."
                    }
                }
            } catch (_: Exception) {
    ...
    private fun startAudioEngine() {
        ...
        }.apply { name = "SequencePlayerAudio11"; priority = Thread.MAX_PRIORITY; start() }
    }
    ...
    override fun onDestroy() { playing = false; audioGeneration++; songGeneration++; clearPreload(); handler.removeCallbacks(progressUpdater); releaseTrack(); super.onDestroy() }
}
