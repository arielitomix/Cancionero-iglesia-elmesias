...
class MainActivityV12 : Activity() {
    companion object {
    ...
    private fun buildUi(): ScrollView {
        ...
        content.addView(TextView(this).apply { text = "SEQUENCE PLAYER · 0.12"; setTextColor(Color.rgb(145,160,178)); textSize = 12f })
        content.addView(TextView(this).apply { text = "Streaming por bloques"; setTextColor(Color.WHITE); textSize = 26f })
    ...
    private fun newSong(){
        songGeneration++;loadingSong=false;stopPlayback();selectedSongIndex=-1;titleInput.setText("")
    ...
    private fun loadSong(index:Int){
        ...
        val song=songs[index];titleInput.setText(song.title);song.stems.forEach{currentStems.add(StemState(it.name,it.uri,null,it.volume,it.muted,it.solo))};renderStemRows();statusView.text="Preparando ${song.title}…"
        Thread{
            try{
                ...
                runOnUiThread{
                    ...
                    currentStems.forEachIndexed{i,stem->stem.info=infos[i]};loadingSong=false;renderStemRows();validateLoadedStems();statusView.text="${song.title} lista.";getSharedPreferences(PREFS,MODE_PRIVATE).edit().putInt(LAST_SONG_KEY,index).apply()
                }
            }catch(e:Exception){runOnUiThread{if(loadGeneration==songGeneration){loadingSong=false;updateControls();statusView.text="No se pudo abrir ${song.title}: ${e.message?:"archivo no disponible"}"}}}
        }.apply{name="SequenceHeaderLoad12";start()}
    }
    ...
    private fun startPlayback(){if(playing||loadingSong||!allLoaded())return;playing=true;playButton.text="❚❚ PAUSA";statusView.text="Reproduciendo por streaming.";startAudioEngine()}
    ...
    private fun startAudioEngine(){
        ...
        Thread{
            ...
            try{
                while(playing&&localGeneration==audioGeneration){
                    ...
                    if(frame>=totalFrames){
                        ...
                        else{playing=false;handler.post{playButton.text="▶ PLAY";statusView.text="Terminó la canción."};break}
                    }
            ...
            }catch(e:Exception){handler.post{if(localGeneration==audioGeneration){playing=false;playButton.text="▶ PLAY";statusView.text="Error de streaming: ${e.message?:"audio"}"}}}
        ...
        }.apply{name="SequenceStreaming12";priority=Thread.MAX_PRIORITY;start()}
    }
