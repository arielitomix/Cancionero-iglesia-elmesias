...
class MainActivityV13 : Activity() {
    ...
    private data class WavInfo(
        ...
        val totalFrames: Int get() = (dataSize / bytesPerFrame).toInt()
    }
    ...
    private data class StreamReader(
    ...
    @Volatile private var continueEnabled = false
    @Volatile private var autoStartAfterLoad = false
    ...
    private fun buildUi(): ScrollView {
        ...
        content.addView(TextView(this).apply { text = "SEQUENCE PLAYER · 0.13"; setTextColor(Color.rgb(145,160,178)); textSize = 12f })
        content.addView(TextView(this).apply { text = "Streaming + encadenado"; setTextColor(Color.WHITE); textSize = 26f })
        ...
        libraryRow.addView(Button(this).apply { text = "ABRIR"; setOnClickListener { val pos=songSpinner.selectedItemPosition; if(pos in songs.indices) loadSong(pos) } }, LinearLayout.LayoutParams(dp(90),dp(48)).apply { marginStart=dp(6) })
        ...
        titleInput = EditText(this).apply { hint="Nombre de la canción"; setTextColor(Color.WHITE); setHintTextColor(Color.rgb(130,140,150)); setSingleLine(true) }
        ...
        content.addView(Button(this).apply { text="GUARDAR / ACTUALIZAR CANCIÓN"; setOnClickListener { saveCurrentSong() } }, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(46)).apply { topMargin=dp(5) })
        ...
        continueButton = Button(this).apply {
            ...
            setOnClickListener {
                ...
                text=if(continueEnabled)"⇥ CONTINUAR: ON" else "⇥ CONTINUAR: OFF"
                statusView.text=if(continueEnabled)"Encadenado activado: al terminar seguirá la siguiente canción." else "Encadenado desactivado."
            }
        }
        content.addView(continueButton, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(46)).apply { topMargin=dp(5) })
    ...
    private fun renderStemRows(){
        ...
        currentStems.forEach { stem ->
            val card=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setPadding(0,dp(4),0,dp(6)) }
            val top=LinearLayout(this).apply { orientation=LinearLayout.HORIZONTAL; gravity=Gravity.CENTER_VERTICAL }
            val label=TextView(this).apply {
                val file=stem.info?.name ?: if(stem.uri!=null)"archivo guardado" else "sin archivo"
                text="${stem.name} · $file"; setTextColor(Color.rgb(205,212,222)); textSize=12f
            }
            top.addView(label,LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f))
            top.addView(Button(this).apply { text="+ WAV"; setOnClickListener { pickFileForStem(stem) } },LinearLayout.LayoutParams(dp(82),dp(40)).apply { marginStart=dp(3) })
            top.addView(Button(this).apply { text="×"; setOnClickListener { if(playing)stopPlayback();currentStems.remove(stem);renderStemRows();validateLoadedStems();statusView.text="Stem eliminado." } },LinearLayout.LayoutParams(dp(44),dp(40)).apply { marginStart=dp(3) })
            card.addView(top)
            ...
            controls.addView(Button(this).apply { text=if(stem.muted)"M ✓" else "M";setOnClickListener { stem.muted=!stem.muted;text=if(stem.muted)"M ✓" else "M" } },LinearLayout.LayoutParams(dp(46),dp(40)).apply { marginStart=dp(3) })
            controls.addView(Button(this).apply { text=if(stem.solo)"S ✓" else "S";setOnClickListener { stem.solo=!stem.solo;text=if(stem.solo)"S ✓" else "S" } },LinearLayout.LayoutParams(dp(46),dp(40)).apply { marginStart=dp(3) })
            card.addView(controls);stemsContainer.addView(card)
        }
    }

    private fun promptAddStem(){
        val input=EditText(this).apply { hint="Ej. Guía, Keys, Guitarras";setSingleLine(true) }
        AlertDialog.Builder(this).setTitle("Añadir stem").setView(input).setPositiveButton("AÑADIR"){_,_->
            val name=input.text.toString().trim();if(name.isNotEmpty()){currentStems.add(StemState(name));renderStemRows();validateLoadedStems();statusView.text="Stem añadido: $name"}
        }.setNegativeButton("CANCELAR",null).show()
    }

    private fun pickFileForStem(stem:StemState){
        pendingStem=stem
        startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply { addCategory(Intent.CATEGORY_OPENABLE);type="audio/wav";putExtra(Intent.EXTRA_MIME_TYPES,arrayOf("audio/wav","audio/x-wav","audio/wave")) },PICK_STEM)
    }
    ...
    override fun onActivityResult(requestCode:Int,resultCode:Int,data:Intent?){
        ...
        val target=pendingStem?:return;pendingStem=null
        ...
        val localGeneration=songGeneration
        statusView.text="Leyendo cabecera de $name…"
        Thread{
            try{
                val info=inspectWav(uri,name)
                runOnUiThread{
                    if(localGeneration!=songGeneration||!currentStems.contains(target))return@runOnUiThread
                    target.uri=uri.toString();target.info=info;renderStemRows();validateLoadedStems();statusView.text="$name listo para streaming."
                }
            }catch(e:Exception){runOnUiThread{if(localGeneration==songGeneration)statusView.text="No se pudo cargar: ${e.message?:"WAV incompatible"}"}}
        }.start()
    }

    private fun newSong(){
        songGeneration++;loadingSong=false;autoStartAfterLoad=false;stopPlayback();selectedSongIndex=-1;titleInput.setText("")
        currentStems.clear();currentStems.addAll(listOf(StemState("Click"),StemState("Drums"),StemState("Bass")))
        currentFrame=0;totalFrames=0;currentTimeView.text="0:00";durationView.text="0:00";seekBar.progress=0;seekBar.max=1
        renderStemRows();updateControls();statusView.text="Nueva canción. Carga sus WAV."
    }
    ...
    private fun saveCurrentSong(){
        ...
        saveSongLibrary();refreshSongSpinner();songSpinner.setSelection(selectedSongIndex);getSharedPreferences(PREFS,MODE_PRIVATE).edit().putInt(LAST_SONG_KEY,selectedSongIndex).apply();statusView.text="Canción guardada: $title"
    }

    private fun moveSelectedSong(delta:Int){
        if(songs.size<2||selectedSongIndex !in songs.indices)return
        val target=selectedSongIndex+delta;if(target !in songs.indices)return
        val tmp=songs[selectedSongIndex];songs[selectedSongIndex]=songs[target];songs[target]=tmp;selectedSongIndex=target
        saveSongLibrary();refreshSongSpinner();songSpinner.setSelection(selectedSongIndex);getSharedPreferences(PREFS,MODE_PRIVATE).edit().putInt(LAST_SONG_KEY,selectedSongIndex).apply();statusView.text="Orden del setlist guardado."
    }

    private fun previousSong(){if(songs.isNotEmpty()){autoStartAfterLoad=false;loadSong(if(selectedSongIndex<=0)songs.lastIndex else selectedSongIndex-1)}}
    private fun nextSong(){if(songs.isNotEmpty()){autoStartAfterLoad=false;loadSong(if(selectedSongIndex<0||selectedSongIndex>=songs.lastIndex)0 else selectedSongIndex+1)}}

    private fun loadSong(index:Int){
        if(index !in songs.indices)return
        val shouldAutoStart=autoStartAfterLoad
        val loadGeneration=++songGeneration
        stopPlayback();loadingSong=true;selectedSongIndex=index;songSpinner.setSelection(index);currentStems.clear();currentFrame=0;totalFrames=0;updateControls()
        val song=songs[index];titleInput.setText(song.title);song.stems.forEach{currentStems.add(StemState(it.name,it.uri,null,it.volume,it.muted,it.solo))};renderStemRows();statusView.text=if(shouldAutoStart)"Encadenando ${song.title}…" else "Preparando ${song.title}…"
        Thread{
            try{
                val infos=song.stems.map{inspectWav(Uri.parse(it.uri),getDisplayName(Uri.parse(it.uri))?:it.name)}
                runOnUiThread{
                    if(loadGeneration!=songGeneration)return@runOnUiThread
                    currentStems.forEachIndexed{i,stem->stem.info=infos[i]};loadingSong=false;renderStemRows();validateLoadedStems();getSharedPreferences(PREFS,MODE_PRIVATE).edit().putInt(LAST_SONG_KEY,index).apply()
                    if(shouldAutoStart&&autoStartAfterLoad){autoStartAfterLoad=false;statusView.text="${song.title} lista · iniciando…";startPlayback()}else{autoStartAfterLoad=false;statusView.text="${song.title} lista."}
                }
            }catch(e:Exception){runOnUiThread{if(loadGeneration==songGeneration){loadingSong=false;autoStartAfterLoad=false;updateControls();statusView.text="No se pudo abrir ${song.title}: ${e.message?:"archivo no disponible"}"}}}
        }.apply{name="SequenceHeaderLoad13";start()}
    }
    ...
    private fun refreshSongSpinner(){
        ...
        songSpinner.adapter=ArrayAdapter(this,android.R.layout.simple_spinner_dropdown_item,labels);if(selectedSongIndex in songs.indices)songSpinner.setSelection(selectedSongIndex)
    }
    ...
    private fun loadSongLibrary(){
        songs.clear();val raw=getSharedPreferences(PREFS,MODE_PRIVATE).getString(SONGS_KEY,"[]")?:"[]"
        try{
            val arr=JSONArray(raw)
            for(i in 0 until arr.length()){
                val o=arr.getJSONObject(i);val stems=mutableListOf<StemPreset>();val newStems=o.optJSONArray("stems")
                if(newStems!=null){for(j in 0 until newStems.length()){val s=newStems.getJSONObject(j);val uri=s.optString("uri");if(uri.isNotEmpty())stems.add(StemPreset(s.optString("name","Stem ${j+1}"),uri,s.optInt("volume",100),s.optBoolean("muted",false),s.optBoolean("solo",false)))}}
                else{
                    val legacy=listOf(Triple("Click",o.optString("clickUri"),o.optInt("clickVolume",100)),Triple("Drums",o.optString("drumsUri"),o.optInt("drumsVolume",100)),Triple("Bass",o.optString("bassUri"),o.optInt("bassVolume",100)))
                    legacy.forEach{(name,uri,volume)->if(uri.isNotEmpty()){val muted=when(name){"Click"->o.optBoolean("clickMuted",false);"Drums"->o.optBoolean("drumsMuted",false);else->o.optBoolean("bassMuted",false)};val solo=when(name){"Click"->o.optBoolean("clickSolo",false);"Drums"->o.optBoolean("drumsSolo",false);else->o.optBoolean("bassSolo",false)};stems.add(StemPreset(name,uri,volume,muted,solo))}}
                }
    ...
    private fun saveSongLibrary(){
        val arr=JSONArray();songs.forEach{song->arr.put(JSONObject().apply{put("title",song.title);put("stems",JSONArray().apply{song.stems.forEach{s->put(JSONObject().apply{put("name",s.name);put("uri",s.uri);put("volume",s.volume);put("muted",s.muted);put("solo",s.solo)})}})})};getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(SONGS_KEY,arr.toString()).apply()
    }

    private fun validateLoadedStems(){
        if(currentStems.isEmpty()){totalFrames=0;updateControls();return}
        val infos=currentStems.mapNotNull{it.info};if(infos.size!=currentStems.size){updateControls();return}
        val rates=infos.map{it.sampleRate}.toSet();if(rates.size>1){statusView.text="Todos los WAV deben usar el mismo sample rate.";updateControls();return}
        sampleRate=infos.first().sampleRate;totalFrames=infos.maxOf{it.totalFrames};val durationMs=((totalFrames.toLong()*1000L)/sampleRate).toInt();seekBar.max=durationMs.coerceAtLeast(1);durationView.text=formatTime(durationMs);currentFrame=0;updateControls()
    }

    private fun allLoaded():Boolean=currentStems.isNotEmpty()&&currentStems.all{it.uri!=null&&it.info!=null}&&currentStems.mapNotNull{it.info?.sampleRate}.toSet().size==1

    private fun startPlayback(){if(playing||loadingSong||!allLoaded())return;playing=true;playButton.text="❚❚ PAUSA";statusView.text=if(continueEnabled)"Reproduciendo · encadenado ON." else "Reproduciendo por streaming.";startAudioEngine()}
    ...
    private fun openReader(info:WavInfo,startFrame:Int,framesPerBlock:Int):StreamReader{
        val pfd=contentResolver.openFileDescriptor(Uri.parse(info.uri),"r")?:error("No se pudo abrir ${info.name}")
        val input=FileInputStream(pfd.fileDescriptor)
        input.channel.position(info.dataOffset + startFrame.toLong()*info.bytesPerFrame)
        return StreamReader(pfd,input,info,ByteArray(framesPerBlock*info.bytesPerFrame))
    }
    ...
    private fun startAudioEngine(){
        ...
        Thread{
            val framesPerBlock=1024;var readers=infos.map{openReader(it,currentFrame,framesPerBlock)};val out=ShortArray(framesPerBlock*2)
            try{
                while(playing&&localGeneration==audioGeneration){
                    var frame=currentFrame
                    if(frame>=totalFrames){
                        if(loopEnabled){closeReaders(readers);currentFrame=0;frame=0;readers=infos.map{openReader(it,0,framesPerBlock)}}
                        else{
                            playing=false
                            val shouldContinue=continueEnabled&&songs.size>1&&selectedSongIndex in songs.indices
                            val nextIndex=if(selectedSongIndex>=songs.lastIndex)0 else selectedSongIndex+1
                            handler.post{
                                playButton.text="▶ PLAY"
                                if(shouldContinue){autoStartAfterLoad=true;statusView.text="Terminó · cargando siguiente…";loadSong(nextIndex)}
                                else statusView.text="Terminó la canción."
                            }
                            break
                        }
                    }
                    val frames=minOf(framesPerBlock,totalFrames-frame);val anySolo=states.any{it.solo};var p=0
                    val bytesRead=IntArray(readers.size)
                    readers.forEachIndexed{i,r->bytesRead[i]=readUpTo(r.input,r.buffer,frames*r.info.bytesPerFrame)}
                    for(i in 0 until frames){
                        var left=0f;var right=0f
                        states.forEachIndexed{si,state->
                            val r=readers[si];val offset=i*r.info.bytesPerFrame
                            var sample=0
            ...
            }catch(e:Exception){handler.post{if(localGeneration==audioGeneration){playing=false;autoStartAfterLoad=false;playButton.text="▶ PLAY";statusView.text="Error de streaming: ${e.message?:"audio"}"}}}
            finally{closeReaders(readers);try{track.stop()}catch(_:Exception){};try{track.release()}catch(_:Exception){};if(audioTrack===track)audioTrack=null}
    ...
    private fun inspectWav(uri:Uri,name:String):WavInfo{
        val pfd=contentResolver.openFileDescriptor(uri,"r")?:error("No se pudo abrir el archivo")
        val input=FileInputStream(pfd.fileDescriptor)
        try{
            ...
            var format=-1;var channels=-1;var rate=-1;var bits=-1;var logicalPos=12L
            while(true){
                val id=readFourCc(stream);val size=readLeInt(stream);logicalPos+=8
                when(id){
                    "fmt "->{if(size<16)error("fmt inválido");format=readLeShort(stream);channels=readLeShort(stream);rate=readLeInt(stream);readLeInt(stream);readLeShort(stream);bits=readLeShort(stream);skipFully(stream,size-16);logicalPos+=size;if((size and 1)==1){skipFully(stream,1);logicalPos++}}
                    "data"->{if(format!=1)error("Solo PCM");if(bits!=16)error("Solo 16 bits");if(channels<1)error("Canales inválidos");if(rate<=0)error("Sample rate inválido");return WavInfo(name,uri.toString(),rate,channels,logicalPos,size.toLong())}
                    else->{skipFully(stream,size);logicalPos+=size;if((size and 1)==1){skipFully(stream,1);logicalPos++}}
                }
            }
        }finally{try{input.close()}catch(_:Exception){};try{pfd.close()}catch(_:Exception){}}
    }

    private fun readUpTo(input:InputStream,buf:ByteArray,len:Int):Int{var total=0;while(total<len){val n=input.read(buf,total,len-total);if(n<0)break;total+=n};return total}
    ...
    override fun onDestroy(){playing=false;audioGeneration++;songGeneration++;autoStartAfterLoad=false;handler.removeCallbacks(progressUpdater);releaseTrack();super.onDestroy()}
}
