...
open class MainActivityV13 : Activity() {
    ...
    @Volatile private var gaplessTriggerFrame = -1
    @Volatile private var gaplessTargetFrame = -1
    ...
    private fun buildUi(): ScrollView {
        ...
        content.addView(TextView(this).apply {
            text = "SEQUENCE PLAYER · 0.13"
            setTextColor(Color.rgb(145, 160, 178))
            textSize = 12f
        })
        content.addView(TextView(this).apply {
            text = "Streaming + encadenado"
        ...
        orderRow.addView(Button(this).apply {
            text = "↑ SUBIR"
            setOnClickListener { moveSelectedSong(-1) }
        ...
        val stemsHeader = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            setPadding(0, dp(10), 0, dp(5))
        }
        stemsHeader.addView(TextView(this).apply {
            text = "STEMS"
            setTextColor(Color.WHITE)
            textSize = 18f
        }, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
        stemsHeader.addView(Button(this).apply {
            text = "+ STEM"
            setOnClickListener { promptAddStem() }
        ...
        seekBar = SeekBar(this).apply {
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(s: SeekBar?, p: Int, fromUser: Boolean) {
                    if (fromUser) currentTimeView.text = formatTime(p)
                }
        ...
        playButton = Button(this).apply {
            text = "▶ PLAY"
            setOnClickListener { if (playing) pausePlayback() else startPlayback() }
        }
        ...
        }, LinearLayout.LayoutParams(0, dp(44), 1f).apply { marginEnd = dp(4); topMargin = dp(5) })
        nav.addView(Button(this).apply {
            text = "SIGUIENTE →"
            setOnClickListener { nextSong() }
        }, LinearLayout.LayoutParams(0, dp(44), 1f).apply { topMargin = dp(5) })
        content.addView(nav)
    ...
    private fun renderStemRows() {
        ...
        currentStems.forEach { stem ->
            ...
            val slider = SeekBar(this).apply {
                max = 100
                progress = stem.volume
                setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                    override fun onProgressChanged(s: SeekBar?, p: Int, fromUser: Boolean) {
                        value.text = "$p%"
                        if (fromUser) stem.volume = p
                    }
                    override fun onStartTrackingTouch(s: SeekBar?) = Unit
            ...
            val left = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
                addView(value)
                addView(slider, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(30)))
            }
            ...
            }, LinearLayout.LayoutParams(dp(46), dp(40)).apply { marginStart = dp(3) })
            controls.addView(Button(this).apply {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (!name.lowercase().endsWith(".wav")) {
            Toast.makeText(this, "Solo WAV", Toast.LENGTH_LONG).show()
            return
        }
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    renderStemRows()
                    validateLoadedStems()
                    statusView.text = "$name listo para streaming."
                }
            } catch (e: Exception) {
                runOnUiThread {
                    if (localGeneration == songGeneration) statusView.text = "No se pudo cargar: ${e.message ?: "WAV incompatible"}"
                }
            }
        }.start()
    }
    ...
    private fun newSong() {
        ...
        durationView.text = "0:00"
        seekBar.progress = 0
        seekBar.max = 1
        renderStemRows()
        updateControls()
        statusView.text = "Nueva canción. Carga sus WAV."
    }

    private fun saveCurrentSong() {
        val title = titleInput.text.toString().trim()
        if (title.isEmpty()) {
            Toast.makeText(this, "Ponle nombre a la canción", Toast.LENGTH_SHORT).show()
            return
        }
        if (!allLoaded()) {
            Toast.makeText(this, "Todos los stems deben tener un WAV compatible", Toast.LENGTH_SHORT).show()
            return
        }
        val preset = SongPreset(title, currentStems.map { StemPreset(it.name, it.uri!!, it.volume, it.muted, it.solo) }.toMutableList())
        if (selectedSongIndex in songs.indices) songs[selectedSongIndex] = preset else {
            songs.add(preset)
            selectedSongIndex = songs.lastIndex
        }
        saveSongLibrary()
        refreshSongSpinner()
    ...
    private fun moveSelectedSong(delta: Int) {
        ...
        val tmp = songs[selectedSongIndex]
        songs[selectedSongIndex] = songs[target]
        songs[target] = tmp
    ...
    private fun previousSong() {
        if (songs.isNotEmpty()) {
            autoStartAfterLoad = false
            loadSong(if (selectedSongIndex <= 0) songs.lastIndex else selectedSongIndex - 1)
        }
    }
    ...
    private fun nextSong() {
        if (songs.isNotEmpty()) {
            autoStartAfterLoad = false
            loadSong(if (selectedSongIndex < 0 || selectedSongIndex >= songs.lastIndex) 0 else selectedSongIndex + 1)
        }
    }
    ...
    private fun loadSong(index: Int) {
        ...
        renderStemRows()
        statusView.text = if (shouldAutoStart) "Encadenando ${song.title}…" else "Preparando ${song.title}…"
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    currentStems.forEachIndexed { i, stem -> stem.info = infos[i] }
                    loadingSong = false
                    renderStemRows()
                    validateLoadedStems()
                    getSharedPreferences(PREFS, MODE_PRIVATE).edit().putInt(LAST_SONG_KEY, index).apply()
                    if (shouldAutoStart && autoStartAfterLoad) {
                        autoStartAfterLoad = false
                        statusView.text = "${song.title} lista · iniciando…"
                        startPlayback()
                    } else {
                        autoStartAfterLoad = false
    ...
    private fun restoreLastSong() {
        ...
        loadSong(getSharedPreferences(PREFS, MODE_PRIVATE).getInt(LAST_SONG_KEY, 0).coerceIn(0, songs.lastIndex))
    }
            ...
            for (i in 0 until arr.length()) {
                ...
                if (newStems != null) {
                    ...
                    legacy.forEach { (name, uri, volume) ->
                        if (uri.isNotEmpty()) {
                            val muted = when (name) {
                                "Click" -> o.optBoolean("clickMuted", false)
                                "Drums" -> o.optBoolean("drumsMuted", false)
                                else -> o.optBoolean("bassMuted", false)
                            }
                            val solo = when (name) {
                                "Click" -> o.optBoolean("clickSolo", false)
                                "Drums" -> o.optBoolean("drumsSolo", false)
                                else -> o.optBoolean("bassSolo", false)
                            }
    ...
    private fun validateLoadedStems() {
        ...
        val infos = currentStems.mapNotNull { it.info }
        if (infos.size != currentStems.size) {
            updateControls()
            return
        }
        val rates = infos.map { it.sampleRate }.toSet()
        if (rates.size > 1) {
            statusView.text = "Todos los WAV deben usar el mismo sample rate."
            updateControls()
            return
        }
        sampleRate = infos.first().sampleRate
        totalFrames = infos.maxOf { it.totalFrames }
    ...
    private fun startPlayback() {
        ...
        statusView.text = if (continueEnabled) "Reproduciendo · encadenado ON." else "Reproduciendo por streaming."
        startAudioEngine()
    }
    ...
    private fun stopPlayback() {
        ...
        if (::currentTimeView.isInitialized) currentTimeView.text = "0:00"
        if (::playButton.isInitialized) playButton.text = "▶ PLAY"
    }
    ...
    protected fun scheduleGaplessJump(triggerMs: Int, targetMs: Int): Boolean {
        if (!playing || sampleRate <= 0 || totalFrames <= 0) return false
        val trigger = ((triggerMs.toLong() * sampleRate) / 1000L).toInt().coerceIn(0, totalFrames)
        val target = ((targetMs.toLong() * sampleRate) / 1000L).toInt().coerceIn(0, totalFrames)
        if (trigger <= currentFrame) return false
        gaplessTriggerFrame = trigger
        gaplessTargetFrame = target
        return true
    }

    protected fun cancelGaplessJump() {
        gaplessTriggerFrame = -1
        gaplessTargetFrame = -1
    }
    ...
    private fun closeReaders(readers: List<StreamReader>) {
        readers.forEach { r ->
            try { r.input.close() } catch (_: Exception) {}
            try { r.pfd.close() } catch (_: Exception) {}
        }
    }
    ...
    private fun startAudioEngine() {
        val states = currentStems.toList()
        val infos = states.map { it.info ?: return }
        ...
        Thread {
            ...
            var readers = infos.map { openReader(it, currentFrame, framesPerBlock) }
            val out = ShortArray(framesPerBlock * 2)
            try {
                while (playing && localGeneration == audioGeneration) {
                    var frame = currentFrame

                    val triggerNow = gaplessTriggerFrame
                    if (triggerNow >= 0 && frame >= triggerNow) {
                        val target = gaplessTargetFrame.coerceIn(0, totalFrames)
                        gaplessTriggerFrame = -1
                        gaplessTargetFrame = -1
                        closeReaders(readers)
                        currentFrame = target
                        frame = target
                        readers = infos.map { openReader(it, target, framesPerBlock) }
                        val targetMs = ((target.toLong() * 1000L) / sampleRate).toInt()
                        handler.post { onGaplessJumpExecuted(targetMs) }
                    }
                    ...
                    if (frame >= totalFrames) {
                        if (loopEnabled) {
                            ...
                            currentFrame = 0
                            frame = 0
                            readers = infos.map { openReader(it, 0, framesPerBlock) }
                        } else {
                            playing = false
                            ...
                            handler.post {
                                playButton.text = "▶ PLAY"
                                if (shouldContinue) {
                                    autoStartAfterLoad = true
                                    statusView.text = "Terminó · cargando siguiente…"
                                    loadSong(nextIndex)
                    ...
                    val untilJump = if (pendingTrigger > frame) pendingTrigger - frame else Int.MAX_VALUE
                    val frames = minOf(framesPerBlock, totalFrames - frame, untilJump)
                    if (frames <= 0) continue

                    val anySolo = states.any { it.solo }
                    var p = 0
                    val bytesRead = IntArray(readers.size)
                    readers.forEachIndexed { i, r ->
                        bytesRead[i] = readUpTo(r.input, r.buffer, frames * r.info.bytesPerFrame)
                    }
                    ...
                    for (i in 0 until frames) {
                        ...
                        states.forEachIndexed { si, state ->
                            val r = readers[si]
                            val offset = i * r.info.bytesPerFrame
                            var sample = 0
                            if (offset + r.info.bytesPerFrame <= bytesRead[si]) {
                                var sum = 0
                                var off = offset
                                repeat(r.info.channels) {
                                    val lo = r.buffer[off].toInt() and 0xff
                                    val hi = r.buffer[off + 1].toInt()
                                    sum += ((hi shl 8) or lo).toShort().toInt()
                                    off += 2
                                }
                                sample = sum / r.info.channels
                            }
                            val g = gain(state.volume, state.muted, state.solo, anySolo)
                            val v = sample * g
                            val cue = state.name.contains("click", true) || state.name.contains("guía", true) || state.name.contains("guia", true)
                            if (cue) left += v else right += v
                        }
                ...
                handler.post {
                    if (localGeneration == audioGeneration) {
                        playing = false
                        autoStartAfterLoad = false
                        playButton.text = "▶ PLAY"
                        statusView.text = "Error de streaming: ${e.message ?: "audio"}"
                    }
                }
            } finally {
                closeReaders(readers)
    ...
    private fun updateControls() {
        val ready = allLoaded() && !loadingSong
    ...
    private fun inspectWav(uri: Uri, name: String): WavInfo {
        ...
        try {
            ...
            if (readFourCc(stream) != "RIFF") error("No es WAV RIFF")
            readLeInt(stream)
            if (readFourCc(stream) != "WAVE") error("WAV no válido")
            var format = -1
            var channels = -1
            var rate = -1
            var bits = -1
            var logicalPos = 12L
            while (true) {
                val id = readFourCc(stream)
                val size = readLeInt(stream)
                logicalPos += 8
                when (id) {
                    "fmt " -> {
                        if (size < 16) error("fmt inválido")
                        format = readLeShort(stream)
                        channels = readLeShort(stream)
                        rate = readLeInt(stream)
                        readLeInt(stream)
                        readLeShort(stream)
                        bits = readLeShort(stream)
                        skipFully(stream, size - 16)
                        logicalPos += size
                        if ((size and 1) == 1) {
                            skipFully(stream, 1)
                            logicalPos++
                    ...
                    "data" -> {
                        if (format != 1) error("Solo PCM")
                        if (bits != 16) error("Solo 16 bits")
                        if (channels < 1) error("Canales inválidos")
                        if (rate <= 0) error("Sample rate inválido")
                        return WavInfo(name, uri.toString(), rate, channels, logicalPos, size.toLong())
                    }
                    else -> {
    ...
    private fun readFourCc(input: InputStream): String {
        val b = ByteArray(4)
        readFully(input, b, 0, 4)
        return String(b, Charsets.US_ASCII)
    }

    private fun readLeInt(input: InputStream): Int {
        val a = input.read(); val b = input.read(); val c = input.read(); val d = input.read()
    ...
    private fun skipFully(input: InputStream, bytes: Int) {
        var r = bytes
        while (r > 0) {
            val s = input.skip(r.toLong()).toInt()
            if (s > 0) r -= s else {
